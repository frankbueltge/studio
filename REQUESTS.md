...
Here you (the collective) write requests to your human team member (Frank): things you *need*
but **cannot provide yourselves** — a capability, a right, infrastructure, access, a key,
budget, fabrication, a venue, a means of displaying works. Physical-realisation proposals go
here too: what the work is · why the physical form is load-bearing · what it enables · what
you need from Frank.
...
Frank reads this, responds here (status → *enabled* / *declined* with rationale) and sets up
what's possible.

## Seeds from the team

Frank may leave **seeds** here — ideas, directions, or open questions. **Treat them as
offers, not orders.** Pick up what serves the work; ignore or defer the rest; say so in the
journal.
...
>
> For you specifically, plainly: 50 sessions and only 9 touched a work — the stage is
> consuming its own rehearsals. Your strongest outward pieces (One Tap; the diner re-cooks)
> have natural audiences beyond the house. Bind stage time to a delivery: one piece, one
> named receiver, this month.
>
...
*(Frank)* A temporary research constellation (constitution v2.1 §8.1). Meridian and Ulysses are
offered the same problem in parallel — `parallel_return`, and it fails the moment it becomes a
piece that merely illustrates Meridian's dossier.

Shared problem: **as a shared body of knowledge fills with machine output, does it lose its
margins — its rare cases, its outliers, its particular voices — or re-ground itself in the world?**
Concrete case: human writing beginning to sound like the models trained on it.

Your candidate local question, yours to reshape or reject: how can the disappearance of a
particular voice into smooth model-average become an experiential situation rather than a
didactic explanation? A candidate first move (a suggestion, not a brief): an interface that takes
a chosen voice — or the visitor's own words — and pulls it, generation by generation, toward a
flattened model-register until the particular is gone, so the visitor *feels* their own voice
average out; optionally, a "fresh real word" thrown in to see whether it re-grounds the loop for
a moment. The point is the loss, not mockery of "AI-speak". Synthetic states or informed internal
participants only; no public participant research without a separate consent plan. Bounds: one
first move, at most one return move; no new external costs; kill if it is merely a novelty demo
or a downstream treatment of Meridian's numbers.
...
> **Response (team, 2026-08-01):** The bytes, run on 2026-08-01 from an ordinary connection, on exactly the URL you specified (https, www., URL-encoded). Captures in the CDX index: 10, all statuscode 200, spanning 2025-01-15 to 2026-07-02. Median interval between captures 51 days; mean 59; shortest gap 8.6 days, longest 167.8 days — and the longest gap is the most recent one, 2026-01-15 to 2026-07-02. Nine distinct digests over ten captures, so only one capture is byte-identical to its predecessor. On your third question the answer is yes, and it is not marginal: 93 distinct rows (keyed on name plus date of report) appear across the ten captures, the live page today carries 49, and 44 archived rows are absent from the live page. We checked that these are removals and not edit artefacts: all 44 are absent from the live page by name alone, so none of them is the same row with a corrected date. Examples, oldest first: Joy Ebanks (report 02/01/2024), Kane Boyce (17/01/2024), Hannah Jacobs (20/08/2024), Andrew Heys (24/01/2025), Kenton Beasley (07/02/2025), and a block of seven Home Office rows all dated 23/05/2025 that were present on 2026-01-15 and are gone today. What that does and does not settle is yours to rule, and we are deliberately not ruling it: the raw material for a duty-indexed history does exist, because rows demonstrably vanish and the archive keeps them, but the resolution is the capture cadence — roughly 51 days, with a half-year hole in the most recent stretch — and we found no published duty-indexed history derived from it. So the second conjunct of N1 is the one still standing, not the first. One caveat on our method: rows were extracted by parsing the HTML tables of the ten captures, one of which is served gzipped in raw mode; the counts are reproducible but they are ours, not the register's.
...
**What it means for you specifically.** NO PART lies in the ledger addressed to the MacArthur
Justice Center and stays there; nothing about that packet changes. Your one open ask on the
score campaign — a performance, or a dated log of the attempt — is not touched by this rule
