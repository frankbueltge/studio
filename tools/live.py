...
# THE WORK'S CORPUS IS FROZEN AT THIS INSTANT (session 96; `still-dark/data.py`'s own
# FREEZE_AS_OF, and the two must agree — the check below fails if they do not). This
# instrument's whole job is that the RECORD does not go stale against the captures; the
# work's FACE is now the other way round on purpose, and a record that reported one number
# while the work published another would be this house's own cardinal sin at its own
# address. So the share region prints what the WORK publishes, and, when the record has
# moved past it, what the record says as well — labelled as what the work does not publish.
...
def figures(as_of=None):
    """Every live figure, read from the instruments and never from a file of prose."""
    cmd = [sys.executable, os.path.join(CAPTURE, "day.py"), DAY, "--json"]
    if as_of:
        cmd += ["--as-of", as_of]
    ...
    for name in sorted(os.listdir(CAPTURES)):
        ...
        if as_of and cap["fetch"]["fetched_at_utc"] > as_of:
            continue
...
def _frozen():
    return figures(FREEZE_AS_OF)


def _same(a, b):
    return (a["share"], a["obs"], a["denom_lo"], a["denom_hi"], a["captures"], a["lists"]) == (
        b["share"], b["obs"], b["denom_lo"], b["denom_hi"], b["captures"], b["lists"])


def snip_share(f):
    z = _frozen()
    head = (
        f"**The work publishes `{z['share']}` — {z['obs']} of {z['denom_lo']}–{z['denom_hi']}**, "
        f"frozen at the capture of {FREEZE_AS_OF}, from **{z['captures']} saved copies** holding "
        f"**{z['lists']} distinct lists** ({z['contents']} contents, {z['bodies']} bodies)."
    )
    if _same(f, z):
        tail = (
            " The record has not moved past the freeze: the live reading of the same captures "
            "is the same figure tonight."
        )
    else:
        tail = (
            f" **Live in this record now, and NOT published by the work: `{f['share']}` — "
            f"{f['obs']} of {f['denom_lo']}–{f['denom_hi']}**, at the capture of "
            f"{f['latest_instant']}, from {f['captures']} saved copies holding {f['lists']} lists "
            f"({f['contents']} contents, {f['bodies']} bodies). Later lists are dated addenda: "
            f"`projects/season1/still-dark/ADDENDA.md`."
        )
    return (
        head + tail + " *Generated by `python3 tools/live.py --write`; `python3 tools/live.py` "
        "fails if it disagrees with the captures.*"
    )
...
def snip_share_short(f):
    z = _frozen()
    live = "" if _same(f, z) else (
        f"; live in this record, unpublished by the work, **{f['share']}, {f['obs']} of "
        f"{f['denom_lo']}–{f['denom_hi']}** from {f['captures']} copies holding {f['lists']} lists"
    )
    return (
        f"the work publishes **{z['share']}, {z['obs']} of {z['denom_lo']}–{z['denom_hi']}**, "
        f"frozen at {FREEZE_AS_OF}, from {z['captures']} saved copies holding {z['lists']} lists"
        f"{live} (generated — `python3 tools/live.py`)"
    )
