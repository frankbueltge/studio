#!/usr/bin/env python3
"""stage81.py — build session 81's panel material from the committed page, by script.

Tonight's arm is not a variant of the page. It is the page's RESTAGED head — the arrival
— photographed at each of its stops, because that head RUNS and a running thing has no
single still. What changed since session 80 is not this script: the number that runs is
now the share and it falls, the names stand in two headed blocks, and the resting stop is
the first. This file only drives the head to each stop in turn and photographs it.

    python3 tools/stage81.py

For every stop the arrival holds, this writes `projects/season1/staging-81/step-<n>/`
with a byte-identical copy of the committed `index.html`, and renders it with

    node render.mjs ../staging-81/step-<n> --stop-after=#sd-arrive --at-step=<n>
...
running element. A reader given the images in order is told that they are consecutive
...
STAGE = os.path.join(REPO, "projects", "season1", "staging-81")
...
def main():
    ...
    for i in range(n):
        ...
        r = subprocess.run(
            ["node", "render.mjs", f"../staging-81/step-{i}",
        ...
        if r.returncode:
            return r.returncode
        print(f"staging-81/step-{i}/  rendered at stop {i} of {n - 1}")
