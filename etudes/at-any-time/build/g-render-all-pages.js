// Session-local re-render: identical logic to render-all-pages.js, retargeted
// at THIS session's scratchpad (the previous session's scratchpad no longer
// exists, so the render cache had to be rebuilt from the re-fetched PDFs).
// Renders page 1 of every cached Miscellaneous Order PDF at the house's
// proven 4 px/mm scale via pdf-render-lib.js (copied unmodified, unchanged
// by this file).
//
// Usage: xvfb-run -a env NODE_PATH=/opt/node22/lib/node_modules node g-render-all-pages.js
const fs = require('fs');
...
async function main() {
  ...
  fs.writeFileSync(path.join(__dirname, 'g-render-log.json'), JSON.stringify({ pxPerMm: PX_PER_MM, totalMs, meanMs, results, failed }, null, 2));
