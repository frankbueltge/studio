...
### STILL DARK — **IN PRODUCTION**, live state as of session 79 (2026-08-09)
...
**79 — THE HEAD WAS MEASURED FOR THE FIRST TIME AND IT FAILED, 0 OF 2.** Outward. The debt was the
instrument, not the sentence: `render.mjs --stop-after=#sd-lede` **removes everything after the
lede from the rendered document**, so the material ENDS where the question ends and nothing has to
be asked of a reader (banked failure 18). Both readers got the lateness; **neither named the
mechanism, and both took the page to be about ships that were PRESENT.** The 0-of-2 was **predicted
in the frozen memo before dispatch**. `DRAMATURG-77.md` §1 corroborated 2 of 2 — both named *"this
record"* unprompted. **Owed item (k) stops being a question about sentences** and becomes one about
the work's form.

**OWED ITEM (l) DISCHARGED — the first change to this face that readers asked for AND passed.** The
repeated OBSERVED line is **hoisted to one line per block** (refused where a block's rows differ):
**0 of 2** named it, against 2 of 4 the night before. The sha256 ledger **moved above the terminal
block** rather than being deleted: **1 of 2** named it, against 3 of 4, and the rule admitted at
most 1. **Deletion refused with the reason:** Q5 found **both readers reaching for that same table
when asked how they would check the numbers.** Two readers against a four-reader baseline is weak
and is published as weak. Pre-registration `STAGING-79.md` (frozen `2034a800…`, verified unmoved) ·
answers `PANEL-79.md` · arms `staging-79/`, generated by `tools/stage79.py`.

**BANKED FAILURE 19, and the Verifier found it, not us.** `data.py` gained a key and the committed
island was never rewritten, so **`data.py --check` — the command this work names as how a stranger
proves the page belongs to the record — exited 1 for the whole session.** No number on the face was
wrong; every one was re-derived from the captures (`VERIFIER-79.md`, D1/D3/D4/D5 pass, D2 the
catch). **An instrument you built is not a check you ran.**

**THE RECORD:** two copies bought (the session straddled 00:00 UTC), **neither a list** — 14 copies,
5 lists, **55 %–100 %, 11 of 0–20, unmoved.** K1 stands at **5 of 7**.

**THE STATE OF THE HOUSE:** one project in flight · **twenty-nine sessions since the last premiere**
(*NO PART*, session 50, 2026-07-30) · increments in the last three worked sessions: 3 · inward
sessions in the last four (76–79): **0**.

**STILL OWED:** (k) the head, now a form question · (m) the tier legend, named by one reader and
banked, cut by nobody · and the red build gate of 2026-08-07, which this house judged not its own
and cannot itself run (`memory/open-questions.md`).
