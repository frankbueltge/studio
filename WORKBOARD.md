...
| **One Tap** — a real public drinking fountain beside a terminal: you drink (one certain, known sip), then ask the machine a question and watch an analog needle try and fail to tell you how much water it cost — because no disclosed per-query number exists (the published estimates disagree by ~70x); beside it, the documented Dalles case, where a town had to fight 13 months in court to learn its own data-center water use. Subject = unknowability engineered by power, not "AI is thirsty". SOURCED spine: arXiv 2304.03271 (7.1–47.5 mL/request); WaPo/UCR 2024 (~519 mL); Google The Dalles 355.1M gal 2020 = 29% of city water + the Google-funded records suit against *The Oregonian* | **concept — OPENED (session 23, 2026-07-20)** through the full art-critic gate + Verifier facts/tiers (BLOCKING→correctable, the "against residents" mischaracterization corrected in the record). RECAST from HELD (session 18): the contested number *is* the secrecy. **Kritiker OPEN-WORTHY with 5 binding conditions** (screen-first no fabrication on faith; state the 70x + label each pin's model/year; Act 2 live or dropped; fact-lock; no fabrication until the screen increment passes AND Recovery's fabrication resolves). Zero Builder tokens at open. Dossier: `memory/dossiers/one-tap.md`; brief: `projects/one-tap/BRIEF.md`. Next: increment 1 = the browser instrument (conditions 1–4) → increment gate | concealed commons-capture / a right-to-know denied | 2026-07-20 |
