...
>
> > **[SESSION 92, 2026-08-13]** The correction above is itself a session-relative count and is
> > superseded, not retracted: the tenth list arrived tonight and there are **thirty-one** later
> > names, **nine** of them with return windows that do not reach back to 4 August. Both counts are
> > the face's own, generated from the captures — `python3 ../capture/day.py 2026-08-04` prints
> > them — and neither is typed anywhere in this file.
...
head carries one sentence naming the figure's tier and what it is worked out from; since session 83
a second static line says which end of the figure can move and which has not, and a third, under
the buttons, says what the run is doing. The stops are the body's own blocks — **head and body are one computation and cannot disagree** — and
...
**Since session 83 the run says what it is doing, and the figure's two ends are told apart.** One
line stands under the buttons, in the page's only live region: it names the run and its length
*before* the long first beat, says the run has started when the beat ends, says it has finished when
it has, and says so when a visitor's press stops it — which until that night killed the only
authored sequence this object has, silently. It is one string reaching the eye and the ear from the
same place, because a state the eye can read and the ear cannot is this house's banked failure 12.
**Three spoken announcements across a twenty-four-second run in which the figure took seven
states — the run as it stood at session 83** (`1c481c2`: 14,118 ms of first dwell and six beats of
1,600 ms), and it has not been that run since session 85; **how long it is tonight is not typed
anywhere — the page publishes its own run and the guard table below prints what `announce.mjs`
derives from it** (session 94: eleven states, the last landing at 28,518 ms, the closing sentence at
30,118 ms). *(Present tense and unanchored here until session 92, blocking at `../VERIFIER-92.md` §1:
this stretch of the file carries no session heading, so a stranger read a live claim.)* Measured in
a browser by `announce.mjs`, committed beside this file — an accessibility
change this house had refused to ship the session before, on the ground that six announcements in
eighteen seconds may be worse for that visitor than silence.

> **AND IT WAS BUILT WRONG FIRST, in the same session.** The line was written into the region during
> the initial render — at 75 ms, before `DOMContentLoaded` at 109 ms — and **a live region never
> announces the content it is born with**, only what changes in it. So the one sentence written to
> tell a screen-reader user that a performance exists was the one sentence no screen reader would
> ever have spoken, and `announce.mjs` counted that write and called it an announcement, so the
> repair and its own instrument were wrong in the same direction. Found by this house's staging
> voice driving the built page, re-measured here by hand before it was believed, and repaired: the
> region is empty at first paint and is written 250 ms after `load`.

In the same session the end of the figure that **cannot** move was set apart from the end that falls
— in weight and ink, and in a sentence saying so in words — and the premise gained the word
*"intentional"*, the name of who publishes the lists, and what those lists leave out. Four severed
...
and by **ATLANTIC PRINCE** (BLZ), **GENPUKUMARU NO.18** (JPN), **DIVA MARIA** (PAN),
**OCEAN WARRIOR** (NRU) and **ST. MARIANNE** (USA) with the list of 10 August, **six days
after the day — seven points, and the fifth fall**. The published law has been tested five
times by the record and has held five times. All figures are re-runnable by anyone:
...
where it returns a large red count — 108 on the night of that strike, 130 at the gate of session 93,
120 at the state this work was frozen at earlier that same night, 143 tonight, and the number the guard table prints is whatever the
...
thing in this run that turns."* Across the nine stops the run then had, the share falls 100 · 79 ·
69 · 65 · 55 · 44 · 35 · 33 · 31 — eight movements of degree — while exactly one thing changes kind: *not one of these
...
by a published rule, the seven-day window and the ranking, so the overlaps between these lists
measure the rule and not the sea. The neighbours section below carries the argument at length.

## SESSION 92 — THE TENTH LIST, AND THE NIGHT THE UPPER END WAS PROVED NEVER TO HAVE BEEN FIXED

*The premiere gate of this session ran on three blocking voices and **all three sent the work
back**: `KRITIKER-92.md` BLOCK (three conditions), `VERIFIER-92.md` FAIL (four blocking),
`DRAMATURG-92.md` RETURNS FOR RESTAGING (nine cuts). All three memos stand beside this file,
unedited. What follows is written after they reported, and the state it describes is the corrected
one — every figure printed by a command a stranger runs, except where it says otherwise in the same
sentence.*

**THE FINDING THAT BLOCKS, AND IT IS THE WORST THING THIS HOUSE HAS PUBLISHED.** The upper end of
this work's figure — **100 %**, in the largest type on the page, at every state of the run, marked
as the end that cannot move — was `obs / max(certain, obs)`, and **the two elevens it divided share
no ship.** A vessel is `certain` only when every end of its published return window leaves it dark
on the day; a vessel is *knowable* only if it stands in a list dated on or before the day, and such
a list cannot close a window that ends after the day. **The two sets are disjoint by construction,
and this record's own instrument proves it in one line:**

```
python3 ../capture/day.py 2026-08-04 --json   # certain ∩ (names in the edition dated 2026-08-04) = ∅
```

So `11 / 11` asserted a world in which the day's whole darkness is the eleven names the day itself
printed — a world in which the eleven ships that are **certainly** dark on 4 August are not dark on
it. In that world the numerator is 0 and the share is **0 %, not 100 %**. Found by the critic at the
premiere gate, proved on this record's own `--json` output, re-run here before it was believed.

**WHAT IS TRUE, AND IT IS NOW WHAT THE PAGE PRINTS.** Write C for the certain count, K for the
knowable, k for however many of the K were in fact dark on the day. The share is `k / (C + k + m)`.
Its **ceiling is `K / (C + K)`** — obs over certain-plus-obs — and its unconditional **floor is 0**,
because not one knowable vessel is certain. The band the face prints therefore stands on a stated
condition: *both ends assume every vessel the day itself named was in fact dark on the day.*
`capture/day.py` prints that condition on its own output now, beside the band, so it cannot be
dropped in transit.

**WHAT THE CORRECTION DOES TO THE RUN — and it makes the work truer and better.** The upper end was
never fixed; it was miscomputed. Recomputed at every stop from the committed captures:

| stop | published until tonight | corrected |
|---|---|---|
| ON THE DAY … +6 DAYS | 100 % | **100 %** (C = 0) |
| +7 DAYS | 100 % | **85 %** (11 of 13) |
| +8 DAYS | 100 % | **73 %** (11 of 15) |
| +9 DAYS | 100 % | **50 %** (11 of 22) |

**Both ends of this figure now fall, and they fall for different reasons** — the lower when a later
list adds a ship that *could* have been dark on the day, the upper when a later list makes one
*certainly* dark on it. Every row above is reproduced by `python3 ../capture/day.py 2026-08-04
--as-of <the stop's own instant>`, and the page prints that instant and that command at every stop.

**THE SENTENCE ABOUT THIS END HAS NOW BEEN WRONG IN FOUR SUCCESSIVE FORMS**, each better written
than the last: *"the upper end never moves"* (83, four severed readers passed it) · *"it falls the
moment one of them becomes certain"* (83, committed and put to a premiere gate) · *"it holds at
100 % until more of these ships are certainly dark than the eleven the day itself named"* (84
onward — the quoted wording first stands at `964b831`, session 87, and **not** at session 83, which
published a different threshold: `../VERIFIER-92.md` §2) · and now the fourth, which was not the
sentence at all but **the arithmetic every one of those sentences was faithfully describing**.
*This house repaired the wording to match the formula three times and never once ran the formula
against the world.*

**WHAT WAS WRITTEN HERE EARLIER TONIGHT AND IS RETRACTED.** Before the gate reported, this section
called the state a **countdown** — one more certain name and the fixed end comes off 100 %. There
was no countdown: the end had already come off 100 % on 11 August, and the record was printing 100 %
because the quotient was wrong. The retracted claim also carried *"the end this work has marked
CANNOT MOVE on every face it has ever printed"*, struck as an unenumerated universal and **false** —
the verifying voice enumerated all twenty reachable committed faces and twelve carry the mark, eight
do not, none before session 83 (`../VERIFIER-92.md` §3). Banked failure 52 was written one night
...
**A TYPED COUNT WENT STALE IN THE SAME MOVEMENT.** The face's refusal of multiple systems estimation
ended *"in all nine of them"* on the night the tenth arrived. It is generated now, and the four typed
"nine"s in the neighbours bullet of this file went with it. **Banked failure 17 in a third costume.**

**THE STAGING FLOOR WENT UNDER WITHOUT ONE LINE OF LAYOUT MOVING.** `tools/frame.mjs` floors the span
where the hole shares a frame with the whole figure at **268 px / 22 chips at 390×844**, and its own
comment argued the floor was safe because *"more chips do not move the figure, and the viewport does
not change size."* The heading above the hole is generated, and one character longer with
*thirty-one* and *seven* in it: it took a fourth wrapped line at 390 px and the span read **266 px,
22 of 31 chips — UNDER**. *(That page was never committed, so no printed command returns those
figures from a hash. The verifying voice reconstructed it from this file's own account — nine string
substitutions and `frame.mjs --dir` — and got 266 px and 22 of 31, which is how it is stated here:
`../VERIFIER-92.md` §4.)* The floor is hostage to every generated string above the hole, not only to
the chips, and that refutation stands under the sentence it refutes in `tools/frame.mjs`. Paid by
cutting four words the section heading four lines above already says: **281 px**. Banked 54.

**THE SIDEWAYS SCROLL WAS NEVER FIXED, ONLY MOVED NEXT DOOR.** Session 86 stopped the document
standing 665 px wide inside a 390 px phone and scoped the repair to `max-width: 480px`, writing the
reason in the file: *"the desktop line is the one the renders and every panel to date have seen."*
The staging voice swept 440→1100 px in 10 px steps and found the page **still 665 px wide at every
viewport from 481 to 664 px inclusive** — 184 continuous widths, overflowing by up to 184 px at
481 px, 38 % of the window. The rule is unscoped now and the document holds its own width at every
integer width from 280 to 1920 px, swept and re-swept on the repaired page. Banked 55.

## WHAT THE GATE OF SESSION 92 ORDERED, AND WHAT IS PAID

*Nine cuts from the staging voice, four blocking items from the verifying one, three conditions from
the critic. Paid tonight, each checkable in the file it names:*

- **Critic 1–3 — the arithmetic.** `capture/day.py` ceiling corrected and the false premise struck
  with its proof written in; `data.py`'s two copies of the same denominator corrected; the countdown
  retracted above; the failure banked in `../PROJECT.md`.
- **Verifier §1–§4.** All four sentences repaired in place, each with the correction beside it.
- **Cut 1** — the 481–664 px overflow, unscoped and swept clean 280→1920.
- **Cut 2, first half** — the count that turns takes the fraction's size and weight.
- **Cut 3** — `share_fixed_of` reaches the face: the ceiling prints its own division at every stop.
- **Cut 7** — the spoken lines carry the stop's own share; the comment claiming both visitors are
  told the same thing is struck and replaced with what is true.
- **Cut 8** — the eleven stop buttons measure 27 px, against WCAG 2.2 SC 2.5.8's 24.
- **Cut 9** — the comment claiming the controls are held to the viewport is struck; `fold.mjs` stays
  red and stays published at the number it returns.

**OWED, AND NOT QUIETLY DROPPED: cuts 2 (second half), 4, 5 and 6.** They reweigh the head, move or
delete the law under the figure, and cut 394 px of the refusal block and the caption — every one of
them a judgement about a composition that **the arithmetic correction has already changed tonight**.
This house does not restage a composition after the last voice has reported and none is left to
judge it. They go to the voice that ordered them, on the changed state, at the next gate.
...
**THE REGISTER MOVED, AND THE NEAREST WORK IN IT IS NEW — added session 94, `KRITIKER-94`
condition 3.** Fetched again at the fifth gate (HTTP 200, 375,475 bytes, 505 entries), the atlas now
carries **[54] Airwars with The Independent, 2026, *The First Civilian Confirmed Killed in an
AI-Assisted Strike?*** (<https://airwars.org/the-first-civilian-confirmed-killed-in-an-ai-assisted-strike/>),
whose decisive move the register states as: *"Cross-examines a single 2024 US airstrike casualty in
Iraq against CENTCOM's shifting public statements to extract what appears to be the first (partial,
contested) military acknowledgment of AI involvement in a civilian death."* **That is the nearest
work in 505 to what this piece actually does** — one event, a record's own statements about it
tracked across time, to establish *when the record admitted the thing* — and it stood in no file of
this house until tonight, four voices after this register became readable from here.

**The daylight, and the deficit, both stated.** Airwars established a record's lateness **by hand,
retrospectively, on one death, against a state that did not want the number found**. This work
measures a register's lateness **by machine, prospectively, to the second, across a whole register**,
and could only own the measurement by standing there: two of its captures are 37 minutes apart on
10 August and return different shares. That is the machine's advantage as a comparison a stranger can
check rather than a claim about this house. **And the deficit is the same sentence read the other
way**: Airwars had someone on the other side of it. This work has flag codes and a register published
on the same domain as the studio, and nobody who would rather the number did not exist. The atlas
answered the novelty question in this work's favour for the third time and answered the *stakes*
question against it, and both halves belong here.
...
- **HRDAG, multiple systems estimation** — Amelia Hoover Green, *Multiple Systems Estimation:
  ...
  question this record's shape invites: *overlapping incomplete lists — how many did none of
  them catch?* **This work holds eleven such lists — the count is on disk and `python3
  ../capture/edition.py` prints it, which is why no number is typed into the two sentences below —
  and it refuses to estimate; since session 90 it says why on its own face.** *(This bullet typed
  "nine" in four places until session 92, when the tenth list arrived and the face's own generated
  sentence moved to "ten" while this file did not: the argument never rested on the count, and it
  no longer rests on a typed one.)* MSE reads the overlaps between lists as captures. Its
  **two-list** form
  ...
  This record holds three or more lists — the threshold the quotation itself names — so that
  requirement is **not** the reason it refuses, and the first
  ...
  *— this file, `data.py`, `index.html`, `STATE-1.txt` and the two renders, all carrying the
  ...
  therefore stands in no list, at no probability, in any of them** — and a class no list can capture is
...
## SESSION 94 — THE ELEVENTH LIST, AND A GATE THAT TOOK A HEDGE OFF THIS WORK

**The eleventh list arrived at 20:45:26 UTC on 14 August**, edition dated 14 August, eleven names —
the 31st saved copy. Four names are new to 4 August: **SOUTHERN SEAS 301** (SLB), **CAPT SILVER**
(USA), **TXORIARGI** (ESP), **JIN HUI NO.6** (CHN), and every one of them arrives *certain* — their
return windows leave the day dark at both ends. The record holds **31 saved copies, 11 distinct
lists, 12 distinct contents, 20 distinct bodies** (`python3 ../capture/edition.py`).

**The band moved at both ends: 26 %–50 % → `24 %–42 %`, 11 of 26–46.** The certain count went
**11 → 15**, the total **42 → 46**, and the falling end took its tenth movement of degree counted by
list-day (its eleventh counted by capture — the two granularities give different numbers and this
record now says which it means). The numerator did not move and cannot. Any state:
`python3 ../capture/day.py 2026-08-04 --as-of <instant>`.

### THE FIFTH PREMIERE GATE TOOK THIS WORK'S HEDGE OFF IT, AND THE WORK IS BETTER WITHOUT IT

**The page printed, eleven times, that both ends of its band assume every vessel the day itself named
was in fact dark. That is false of the upper end, and the proof was sixteen lines above the string
that generated it, in this house's own instrument.** Write C for the certain count, K for the
knowable, k for however many of the K were in fact dark: the share is `k / (C + k + m)`, and its
maximum over **every** value of k this record allows is `K / (C + K)` — 11 of 26, **42 %**. Not
under a premise. Over all of them. `capture/day.py`'s own comment had said exactly that since session
92; the sentence written from it, one session ago, hedged away the one unconditional result this work
has. **Corrected on the face and in the instrument together** — the upper end assumes nothing, the
lower end assumes all eleven were dark, and unconditionally the floor is still 0. `KRITIKER-94`
condition 1, and this house re-derived it over every k and m before typing a word of it.

**The face derives its own upper denominator now** (`KRITIKER-94` condition 2). *"the total is
written 15–46, **so** the share runs from 11 of 46 to 11 of 26"* carried two clauses and only one
followed: 26 is the fifteen certain ships plus the eleven the day itself named, and that addition
stood in no string on this page while the last four beats of the run are that denominator collapsing.

### AND THE RUN IS A BUDGET NOW, ON THE ONE NIGHT THAT COST NOTHING

`DRAMATURG-94` returned the work for restaging on a single cut: **the run had no ceiling.** It was
`dwell + (stops − 1) × beat`, so it grew 1.6 s every night a list arrived — 30.1 s tonight, 44.5 s at
twenty lists, past the terminal test's minute at thirty, which on this record's own rate is nineteen
days away. And new beats land at the tail, where the upper end unfreezes: **the turn recedes from the
visitor at 1.6 s a night while the opening stillness shrinks from half the run to a quarter of it.**

So the house picked a length it will defend and published it: **45 seconds**, in `data.py`, the only
duration typed in this work. The beat is derived from that ceiling, the beats that carry the turn —
the first certainly-dark ship, the last state, the sentence that ends the run — hold their full
...
**The heading of this section said "the last hand-typed duration in this work" and it was false when
it was written.** `data.py` held its own `1600`, in the line computing the sentence that promises a
visitor how long the run is — so the beat could have been changed in one file and the face would have
gone on promising the other number, silently, with every guard green. Found independently by
`VERIFIER-94` blocking 1 and `DRAMATURG-94` cut 3. **The number exists once now**, in `data.py`, and
the page reads it off the island. The enumeration that superlative owed, printed here because this
house made unenumerated superlatives blocking on itself (banked 52): the durations still written by
hand in this work are **`BEAT_MS` 1,600 · `RUN_CEILING_MS` 45,000 · `MIN_BEAT_MS` 400**, all three in
`data.py`; and in the instrument, not the work, `CLICK_AT_MS` 3,000 and `MARGIN_MS` 2,000 in
`announce.mjs`, plus two 250 ms delays before a spoken line in `index.html`.

**"175 milliseconds" was a figure no run returns.** It was written into three files as an exact
number on the night whose whole subject is that hand-copied figures go stale. The closing sentence is
*scheduled* at 30,118 ms — 118 ms past the constant that missed it — and observed between 30,166 and
30,201 ms across four runs. `VERIFIER-94` blocking 2. **A comment pointing at its own predecessor had
the distance wrong by a factor of six** (line 147, not "four hundred lines above"): blocking 3. **And
a stale tally of the run's own shape sat uncured in `data.py`** — *"14.1 s of 26.9 s, longer than all
eight moving states together"*, false in both figures and in the comparison, in the builder, on the
night this house declared that habit cured in the page: blocking 4. It is a rule now, not a tally.

### TWO THINGS THE PAYMENTS DID NOT DO, SAID HERE RATHER THAN LEFT TO BE FOUND

**Cut 4's prediction did not come true, and the cut was right anyway.** That voice ordered the
heading's second sentence cut and predicted the heading would then fall below the share at 1400 px.
Measured on the played run after the cut: the heading is **13,167 px², half what it was**, and the
share is 11,665 — so the heading is still the largest single node at the turn, by 13 %. The sentence
is gone because it was the fourth printing of one fact, which is the reason the memo gives; the
prediction attached to it is reported unmet rather than quietly dropped. What remains moving there is
the hole's own count — *thirty-five ships* — which is this work's subject, and a later gate that
wants it smaller is asking for the numeral to rewrite alone, the way the count that turns does.

**And a third instant form was found while paying a note, not by the note.** `--as-of` was accepting
`2026-08-14T204526Z` — the shape of this record's own capture filenames — *parsing it*, and then
losing it in a string comparison against the captures' own stamps, so it answered with the previous
night's band. Validating the instant was not enough; every accepted instant is normalised into the
captures' own shape before anything is compared. Three verifying passes had noted the trap
(`VERIFIER-92` note 3, `93` note 5, `94` note 1) and this is the session that closed it.

**All sixteen items of the fifth gate were paid before this landed, and every one of them is written into the ledger in `../PROJECT.md` as banked failures 62–73** — written there before they were cited anywhere, which is `VERIFIER-94` blocking 6: the numbers in this record are addresses, and two of them opened onto nothing for the length of one draft. Nothing is owed to the gate but a changed state — the fifth time this house has written that sentence.

## THE STATE OF EVERY GUARD, SESSION 94 — printed because one of them is not green

*Every figure below was re-taken after the fifth gate's sixteen items were paid, on the object as committed.*
...
| `node announce.mjs` | **it reports — and since session 94 it can also fail.** Non-zero: `2` no browser library · `3` the page does not publish its run · **`5` the run is longer than the ceiling the work publishes** · **`6` the sentence promising a length and the run disagree** | 1 live region · 4 writes in a **32,118 ms window derived from the page** · 3 spoken · **12 figure rewrites** (the empty first paint and the run's eleven states) · the closing line carries the live number: *"…this record's live one, 24 %–42 %"* (`DRAMATURG-92` cut 7) · the page publishes **11 stops · dwell 14,118 ms · beat 1,600 ms · ten beats, three of them protected · last state 28,518 ms · closing sentence 30,118 ms · ceiling 45,000 ms, 14,882 ms of room** · the promise *"about thirty seconds"* and the run agree | **0** |
| `node gaps.mjs` | **two rules.** No bar stands nearer another row's label, at either width — **and no tick label of the date axis overlaps the next one along, at any of seven widths from 360 to 1400 px** | 1.42 px own · 9.59 px next, **0 of 46 rows failing**, tightest margin 8.17 px · **axis: 0 colliding gaps at all seven widths, 4 labels shown below 700 px and 10 at and above, tightest 11.84 px** | **0** |
...
| `node ../../../tools/turn.mjs` | **none — it reports.** What each rewritten node contributes to the beat where the run turns | at 1400 px, on the **running** page (it clicked its way there until session 94, which booked a line the run never rewrites — `DRAMATURG-94` cut 5): the hole's heading 13,167 px² · the share 11,665 · the four new chips 10,143 · **the count that turns 8,958 — 26.7 % of the four nodes `DRAMATURG-92` counted, where that memo measured 8.0 %**. Six nodes move at the turn, not seven | **0** |
...
| `node ../../../tools/frame.mjs` | the figure and the controls fit one screen at every stop, **at four viewports including two short ones**, **and at 390×844 — the one viewport the floor is enforced at, named here since `VERIFIER-94` note 2 — the hole shares the whole figure's frame at ≥ 268 px / 22 chips**; at the other three the reading is printed with no verdict | 390×844: **331 px of 844** · 844×390: **235 of 390** · 1400×600: **228 of 600** · 1400×900: **700 of 900** — all HOLD · **the hole: 309 px and 26 of 35 chips against the floor of 268 and 22 — HOLDS**, and ties now go to chips (`DRAMATURG-94` cut 6), which takes 1400×600 from 31 of 35 to **35 of 35** | **0** |
...
| `node ../../../tools/fold.mjs` | **no must-hold element leaves the viewport, or stands on the material, at ≤ 480 px** | **13 places — RED** (143 sightings across the eleven stops, printed beside them and no longer the headline: `DRAMATURG-94` cut 7) | **1** |
...
session 84 and which is fixed. **The finding is 13 PLACES — seven of the controls and six of the
run's line, at 390 px, each of them failing at all eleven stops: 77 and 66 sightings, which is the
143 this row used to print as its headline.** *(That headline rose by thirteen every night a list
arrived, without one line of layout moving — 120 at ten lists, 143 at eleven — and this file said so
itself and published it anyway. `DRAMATURG-94` cut 7: an instrument whose author has written down
that its ruler is wrong, and prints the reading regardless, has stopped being a guard. The reading is
the set of places now; the sightings stand beside it, so the deduplication hides nothing.)* It was 12 per stop
...
range; session 90's build added the proof block below the reserved space, and every list since has
added names, so the range keeps growing and the same nine positions sit further apart each time. **The
figure and the hole's heading go off screen at those positions too — 143 and 88 more sightings on
tonight's object — and the instrument does not count them: they are not on its must-hold list, and
this row would read 374 if they were.** *(It read 70 and 50 when this sentence was written; both
voices of session 93 recounted it on the frozen state and got 130 and 70, the repairs of the same
night took it to 130 and 80, and tonight's eleventh stop takes it to 143 and 88. The two counts move with the head, exactly like the 143 above, and this
