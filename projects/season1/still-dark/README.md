...
*(Since session 95 that principle is enforced rather than kept: the paragraph above this one is
generated from the captures by `tools/live.py --write`, and `python3 tools/live.py` fails if any
figure in it, or an unstamped superseded figure anywhere in this record, disagrees with them.)*
...
## SESSION 95 — THE TWELFTH LIST, AND THE RECORD AROUND THIS WORK STOPS BEING TYPED
...
it and false by the time anyone read it (63, 64, 65, 66, 67, 71) — and every one of them was in this
record rather than on the page, because `data.py --check` guards the page and nothing guarded the
record. **`tools/live.py` is that guard**: the live figures in `PROJECT.md`, in `WORKBOARD.md` and at
the head of this file are written by `--write` and fail `python3 tools/live.py` the moment they
disagree with the captures, and every superseded figure anywhere in these four files must carry the
instant, session, transition or hash it was true of, or the run exits 1. On its first run it named
two stale sentences and nothing else — one of them the paragraph in the session-94 block below,
which said *"the record holds"* in the present tense about a count one list old.
...
seconds"* to *"about thirty-two seconds"*, because session 94 made that sentence derive from the run
it describes. The last thing this record types by hand about its own live state is the guard table
above, which needs a browser to produce and so is outside `live.py`'s reach. **It is named here as
the remaining hand-typed live surface rather than left to be found.**
...
| `python3 ../../../tools/live.py` | **new in session 95.** Every live figure in this record is generated from the captures, and every superseded figure carries the instant it was true of | **4 regions marked, 0 disagreeing with the captures · 42 superseded figures, 0 of them unstamped.** It found two stale sentences and a disagreement with this page's own run the moment it was first run | **0** |
