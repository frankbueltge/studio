...
proved that true of one:** 63 and 65 were HTML comments *in `index.html`*, which `data.py --check`
passes clean because it guards the data island and not the markup around it; 64 stood in three files
at once, of which only one was the record; 66 was in the builder, `data.py`; 71 was not hand-typed at
...
run it describes (`DRAMATURG-89.md` cut 3, and the builder's own comment at `data.py` names it).
...
`data.py --check`, `tiers.mjs`, `renders.py` and `live.py` itself — so a browser is not what puts
...
**OWED, and named rather than softened:** the critic's **condition 2** — the face must show what the
...
(`python3 ../capture/edition.py`, which prints tonight's instead).
...
## THE STATE OF EVERY GUARD, SESSION 95 — printed because one of them is not green

*Every figure below was re-taken at the sixth premiere gate, 15 August 2026, on the object as
committed (`index.html` sha256 `52215bf9…`). **This heading and this dateline said SESSION 94 and
...
| `node announce.mjs` | **it reports — and since session 94 it can also fail.** Non-zero: `2` no browser library · `3` the page does not publish its run · **`5` the run is longer than the ceiling the work publishes** · **`6` the sentence promising a length and the run disagree** | 1 live region · 4 writes in a **33,718 ms window derived from the page** · 3 spoken · **13 figure rewrites** (the empty first paint and the run's twelve states) · the closing line carries the live number: *"…this record's live one, 22 %–38 %"* (`DRAMATURG-92` cut 7) · the page publishes **12 stops · dwell 14,118 ms · beat 1,600 ms · eleven beats, three of them protected · last state 30,118 ms · closing sentence 31,718 ms · ceiling 45,000 ms, 13,282 ms of room** · the promise *"about thirty-two seconds"* and the run agree — **and the promise moved by itself tonight**, from *thirty* to *thirty-two*, because session 94 made it derived | **0** |
